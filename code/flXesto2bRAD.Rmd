...
echo readCounts.sh -e .fastq -o xestoRaw >>rawReads
...
ngsAdmixLauncher.py -f xestoNoClones.beagle.gz --maxK 15 -r 20 -n flXestoNoClones --launcher -e reckert2017@fau.edu
...
ngsAdmixLauncher.py -f xestoMain.beagle.gz --maxK 15 -r 20 -n flXestoMain --launcher -e reckert2017@fau.edu
...
```{r, format Clumpak}
logs <- as.data.frame(read.table("flXestoNoClonesNgsAdmixLogfile"))

#output is organized with 10, 11 preceding 1, 2, 3 etc.
logs$K <- c(rep("10", 20), rep("11", 20), rep("12", 20), rep("13", 20), rep("14", 20), rep("15", 20), rep("1", 20), rep("2", 20), rep("3", 20),
rep("4", 20), rep("5", 20), rep("6", 20),
    rep("7", 20), rep("8", 20), rep("9", 20))

write.table(logs[, c(2, 1)], "flXestoNoClonesNgsAdmixLogfile_formatted", row.names = F, col.names = F, quote = F)
...
```{bash}
cat flXestoNoClonesNgsAdmixLogfile_formatted | wc -l
...
Repeat these steps as necessary for all ```ngsAdmix``` runs
```{bash, admixture}
...
echo BA3SNP -v -i 10000000 -b 2000000 -n 1000 xestoMainBA3.txt >> BATest
...
echo '#!/bin/bash' > BATest
echo BA3SNP -v -i 10000000 -b 2000000 -n 1000  -m 0.2 -a 0.6 -f 0.04 xestoMainBA3.txt >> BATest

sbatch -e BATest.e%j -o BATest.o%j -p shortq7 --mail-user reckert2017@fau.edu --mail-type=ALL BATest
#  -2371745.64 % done: [0.00] % accepted: (0.22, 0.01, 0.30, 0.21, 0.80)

echo '#!/bin/bash' > BATest
echo BA3SNP -v -i 10000000 -b 2000000 -n 1000  -m 0.15 -a 0.7 -f 0.03 xestoMainBA3.txt >> BATest
...
echo BA3SNP -v -u -s $RANDOM -i 10000000 -b 2000000 -n 1000   -m 0.15 -a 0.7 -f 0.03 -t -o flXestoBA${i}Out.txt ../xestoMainBA3.txt >> BayesAss$i.sh;
mkdir run$i;
