---
title: "SEFL *Xestospongia muta* Population Genetics"
author: "Ryan Eckert"
date: "2024-06-25"
output:
  html_document:
...
hetK4Letters = data.frame(x = factor(c("Xm1.1", "Xm2", "Xm1.2", "Xm1.3","Admixed")), y = c(0.0025, 0.0025, 0.0025, 0.0025, 0.0025),  grp = c("a", "b", "c", "ac", "cd"))
...
```{r, fst}
pop.order = c("Xm1.3", "Xm1.2", "Xm2", "Xm1.1")
...
for(i in 1:length(fileList)){
  ...
  print(paste(fileList[i], bayesian_deviance(get(fileList[i]), burnin = 2000000, sampling.interval = 1000)))
}
...
```{r, BA3}
bayesAss = read.delim("../data/bayesass/BA3trace.04.txt") %>% filter(State > 2000000) %>% dplyr::select(-State, -LogProb, -X)
...
pcAdmixMain = read.table("../data/admix/main/K3main.output") %>% dplyr::select(V6, V7, V8) %>% dplyr::rename("Xm1.1" = "V6", "Xm1.2" = "V7", "Xm1.3" = "V8") %>% cbind(bamsMain)
...
mainPlots = (mainTree)/((pcaPlotMainS + pcaPlotMainK3 + guide_area()) +
  plot_layout(widths = c(1, 1,0.25), guides = "collect")) +
  plot_layout(heights = c(2.5, 1.75)) +
...
ggsave("../figures/FigureS1.png", plot = mainPlots, height = 10, width = 12, units = "in", dpi = 300)
