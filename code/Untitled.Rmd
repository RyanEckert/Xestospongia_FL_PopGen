...
xestoKColPal = c( "#61003A", "#F32BA3", "#FF477E", "#F46666", "#FF8F8F", "#AA156E")

colorRampPalette(c("#61003A","#F32BA3"))(3)
```
...
```{r, map data}
flSites = read.csv("../data/flXestoMetaData.csv", header = TRUE)
...
## admixture K = 2
#-------------------------------------
K2ad = read.table("../data/admix/ngsAdmix/K2.output") %>% dplyr::select(V6, V7) 
...
## admixture K = 3
#-------------------------------------
K3ad = read.table("../data/admix/ngsAdmix/K3.output") %>% dplyr::select(V6, V7, V8) 
...
## ngsAdmix K = 4
#-------------------------------------
K4ad = read.table("../data/admix/ngsAdmix/K4.output") %>% dplyr::select(V6, V7, V8, V9) 
...
## admixture K = 5
#-------------------------------------
K5ad = read.table("../data/admix/ngsAdmix/K5.output") %>% dplyr::select(V6, V7, V8, V9, V10)
...
## ADMIXTURE K = 4
#-------------------------------------
admixture4 = read.table("../data/admix/admixture/xestoNoClones.4.Q")
admixtureK4 = bams %>%  
  dplyr::select(sample, "site" = region, "depth" = depthZone, "depthm" = depthM) %>% 
  cbind(admixture4) %>% 
  dplyr::rename("Xm1" = "V4", "Xm2" = "V1", "Xm3" = "V3", "Xm4" = "V2") %>% 
  select(order(colnames(.)))
admixture_melt = melt(admixtureK4, id = c("sample", "site", "depth", "depthm"))
...

{
  ...
  p1 = ggtree(tr, layout = "rectangular", size = 0.35) #+ ggtree::geom_tiplab()
...
pcaPlotK4A = ggplot() +
  ...
  geom_point(data = pcangsd, aes(x = PC1, y = PC2, fill = clusterK4, shape = depth), color = "black", size = 3.5, alpha = 1, show.legend = TRUE) +
  scale_shape_manual(values = c(21, 23), name = "Depth Zone") +
...
pcaPlotK2A = ggplot() +
  geom_hline(yintercept = 0, color = "gray90", size = 0.25) +
  geom_vline(xintercept = 0, color = "gray90", size = 0.25) +
  geom_point(data = pcangsd, aes(x = PC1, y = PC2, fill = clusterK2, shape = depth), color = "black", size = 3.5, show.legend = FALSE) +
  scale_shape_manual(values = c(21, 23), name = "Depth Zone") +
...
```{r, pca 1 plot}
pcaPlots = (pcaPlot12S | pcaPlotK4 | pcaPlotK2) +
...
```{r, het plots}
hetPlotK2A = ggplot(data = xestoHet, aes(x = clusterK2, y = k2Het)) +
  # geom_violin(aes(fill = clusterK2, group = clusterK2), adjust = 1, linewidth = 0, color = "black", alpha = 0.35, width = 0.9, trim = F, scale = "width") +
  geom_violin(aes(fill = clusterK2, group = clusterK2), adjust = 1, linewidth = 0.4, color = "black", alpha = 1, trim = F, scale = "area", width = 1.2) +
  # geom_beeswarm(aes(fill = clusterK2), shape = 21, size = 2, cex = 0.5, alpha = 0.5, priority='density') + 
...
hetPlotK4A = ggplot(data = xestoHet, aes(x = clusterK4, y = k4Het)) +
  # geom_violin(aes(fill = clusterK4, group = clusterK4), adjust = 1, linewidth = 0, color = "black", alpha = 0.35, width = 0.9, trim = F, scale = "width") +
  geom_violin(aes(fill = clusterK4, group = clusterK4), adjust = 1, linewidth = 0.4, color = "black", alpha = 1, trim = F, scale = "area", width = 1.2) +
  # geom_beeswarm(aes(fill = clusterK4), shape = 21, size = 2, alpha = 0.25, priority='density') + 
...
fstHeatmapA = ggplot(data = fst %>% filter(Fst != 0), aes(Pop, Pop2, fill = as.numeric(as.character(Fst)))) +
  ...
  geom_segment(data = fst, aes(x = 0.475, xend = 0.2, y = Pop2, yend = Pop2, color = Pop2), size = 24) + #colored boxes for Y-axis labels
  geom_segment(data = fst, aes(x = Pop, xend = Pop, y = 0.2, yend = 0.475, color = Pop), size = 27.5) + #colored boxes for X-axis labels
...
```{r, het plot combined}
hetPlots = hetPlotK2 + hetPlotK4 + fstHeatmap +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = 20),
        legend.text = element_text(size = 8),
        legend.title = element_text(size = 10))
...
ggsave("../figures/figure4.png", plot = hetPlots, width = 13, height = 4, units = "in", dpi = 300)
...
flXestoAdmixK3Melt = melt(flXestoAdmixK2, id.vars=c("sample", "site", "depth", "depthm", "ord"), variable.name="Ancestry", value.name="Fraction")
...
maAFD = as.matrix(read.table("../data/xestoAFD.ibsMat")) # reads in IBS matrix produced by ANGSD
...
snps = snps.a %>% bind_rows(snps.b) %>% group_by(chrom) %>% summarize(n = n()) %>% filter(n == 2) %>% select(!n)
...
```{r, AFDplots}
xmAFD = (afdTree) /((pcaPlot1AFD + afdPlot) + plot_layout(widths = c(3, 1))) +
...
ggsave("../figures/Figure3.png", plot = xmAFD, height = 8, width = 10.5, units = "in", dpi = 300)
```
